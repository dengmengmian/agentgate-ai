...
- Route Profile 配置多 Provider 优先级链
...
- Codex 一键配置写入（config.toml + auth.json）
- Claude Code 一键配置写入（settings.json inline token）
- 配置自动备份与恢复
- Gateway 本地访问令牌（ag*local*\*）认证
...
|                  仪表盘                   |              工具配置               |
| :---------------------------------------: | :---------------------------------: |
| ![仪表盘](docs/screenshots/dashboard.png) | ![工具](docs/screenshots/tools.png) |
...
- `~/.codex/config.toml` — Provider 设置
- `~/.codex/auth.json` — AgentGate 本地令牌

Codex 配置后直接使用，不需要额外设置环境变量。
...
重启终端后 Claude Code 即可通过 AgentGate 使用。
...
1. 添加多个 Provider 到 Provider Chain
2. 调整优先级（数字越小优先级越高）
3. 切换模式：手动 / 故障转移
...
├── src/                          # 前端 React
│   ├── app/App.tsx               # 应用入口
│   ├── pages/                    # 页面（仪表盘/工具/服务商/路由/网关/日志/诊断/设置）
│   ├── components/               # UI 组件
...
│       ├── storage/              # SQLite 存储层
│       ├── models/               # 数据模型
│       ├── tools/                # 工具配置管理（Codex/Claude Code）
│       ├── security/             # 认证与脱敏
│       ├── diagnostics/          # 诊断与自检
