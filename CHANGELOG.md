...
为多 provider 路由补一层「网关行为修正」与节点测速。默认全部关闭，开关全关时和之前完全一样。

### 新增

- **节点测速** —— Providers 页新增「测速」按钮，对所有启用 provider 并行发一个 1-token 的探测请求，按延迟从快到慢列出连接 / TTFB / 总耗时，并染色（< 500ms 绿 / 500-1500ms 黄 / > 1500ms 红）。手动触发，会消耗少量 token，不会自动跑。
- **网关精炼层（3 个可选开关，默认全关）** —— 在「设置 → 通用 → 网关精炼层」打开后，让网关帮你处理常见兼容性问题：
  - **请求字段过滤**：自动剥掉 provider 不支持的字段（如 DeepSeek 的 `web_search`），少踩 400。
  - **推理参数校正**：自动把 `thinking.budget_tokens` / `reasoning.effort` 调到 provider 接受的范围。
  - **错误响应归一**：把 provider 错误改写成客户端协议期望的形态。
  - 上面三项按内置的 DeepSeek / MiMo / Anthropic / OpenAI / Kimi 默认规则工作，全局一个开关控制，关掉就和之前一样字节级透明。
  - 「请求字段过滤」和「推理参数校正」已经接入到 `/v1/responses` / `/v1/messages` / `/v1/chat/completions` / Gemini 几条转换路径，开开关后请求转发时真的会改写。「错误响应归一」当前 commit 还没接到响应路径，下一版加上。
