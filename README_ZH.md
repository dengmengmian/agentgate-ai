...
- MiMo 一等公民：5 个聊天模型（`mimo-v2.5-pro` / `mimo-v2-pro` / `mimo-v2.5` / `mimo-v2-omni` / `mimo-v2-flash`）、多轮 `reasoning_content` 回环、`sk-*` / `tp-*` key 自动切到对应小米 host、`webSearchEnabled` 友好错误提示
- `[1m]` 长上下文后缀：Claude Code 透传路径支持 MiMo / DeepSeek 显式模型 ID（如 `mimo-v2.5-pro[1m]` / `deepseek-v4-pro[1m]`）；AgentGate 默认原样发送模型，只有你在模型或模型映射里配置 `[1m]` 时才会带上。
...
|---|---|---|---|
| 小米 MiMo | `mimo` | Chat + Anthropic | 多轮 `reasoning_content` 回环、`tp-*` host 自动切换、思考态剥 temperature、tool_choice 非 auto 剥除、omni web_search 剥除、CC 路径显式 `[1m]` 模型后缀透传、web_search builtin 按矩阵翻译、`webSearchEnabled` 友好提示 |
| DeepSeek | `deepseek` | Chat + Anthropic | 图片剥离（纯文本模型）、reasoning 注入、schema 清洗、消息重排、CC 路径显式 `[1m]` 模型后缀透传 |
