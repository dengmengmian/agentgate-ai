...
- `[1m]` 长上下文后缀：Claude Code 透传路径上对 DeepSeek 的 1M-capable 模型透明追加。MiMo 官方 Claude Code 路径支持显式模型 ID（如 `mimo-v2.5-pro[1m]`）；AgentGate 默认原样发送 MiMo 模型，只有你在模型或模型映射里配置 `[1m]` 时才会带上。
