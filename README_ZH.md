...
- 支持**小米 MiMo**、DeepSeek、OpenAI、Anthropic、OpenRouter、Kimi、MiniMax、自定义 OpenAI 兼容接口
...
| 概览 | 服务商 |
|:---:|:---:|
| ![概览](docs/screenshots/dashboard.png) | ![服务商](docs/screenshots/providers.png) |
...
| Provider   | 类型                       | 转换方式                | 专属处理 | Vision（按模型） |
| ---------- | -------------------------- | ----------------------- | -------- | ------ |
| 小米 MiMo  | `mimo`                     | Chat Completions / Anthropic 透传 | 多轮 `reasoning_content` 回环、`tp-*` host 自动切换、v2.5-pro/v2.5 思考态剥 temperature、tool_choice 非 auto 剥除、omni web_search 剥除、CC 路径 `[1m]` 后缀注入、web_search builtin 按矩阵翻译、`webSearchEnabled` 友好提示 | `mimo-v2.5` / `mimo-v2-omni` ✓；其他 ✗ |
| DeepSeek   | `deepseek`                 | Chat Completions 转换   | 图片剥离、reasoning 注入、schema 清洗、消息重排、CC 路径 v4-pro `[1m]` 后缀 | 取决于模型 |
...
| Kimi / Moonshot | `kimi`                | Chat Completions 转换   | `web_search` → `builtin_function`/`$web_search`、thinking 控制 | `kimi-for-coding` ✓；其他取决于模型 |
...
├── src/                          # 前端 React
│   ├── app/App.tsx               # 应用入口
│   ├── pages/                    # 页面（概览/快速配置/服务商/路由/网关/客户端/日志/诊断/设置）
│   ├── components/               # UI 组件（布局/通用/仪表盘/服务商/日志/工具/引导）
