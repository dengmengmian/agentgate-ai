...
export function GhostPet({ state }: Props) {
  ...
  return (
    <svg width="100" height="120" viewBox="0 0 100 115" fill="none" xmlns="http://www.w3.org/2000/svg">
      {/* Body */}
      <path d={`M20,90 L20,45 Q20,15 50,15 Q80,15 80,45 L80,90 Q76,98 68,90 Q60,82 52,90 Q44,98 36,90 Q28,82 20,90Z`} fill={bodyColor}>
        {state === "active" && (
      ...
      {state === "sleep" ? (
        <>
          <path d="M35,48 Q39,52 43,48" stroke={eyeColor} strokeWidth="2" fill="none" strokeLinecap="round" />
          <path d="M55,48 Q59,52 63,48" stroke={eyeColor} strokeWidth="2" fill="none" strokeLinecap="round" />
        </>
      ) : (
        <>
      ...
      {/* Mouth */}
      {state === "error" ? (
        <path d="M43,62 Q49,58 55,62" stroke={eyeColor} strokeWidth="1.5" fill="none" strokeLinecap="round" />
      ) : state === "active" ? (
        <ellipse cx="49" cy="62" rx="5" ry="4" fill={eyeColor} opacity="0.6" />
      ) : (
        <path d="M44,61 Q49,66 54,61" stroke={eyeColor} strokeWidth="1.5" fill="none" strokeLinecap="round" />
      )}

      {/* Little arms */}
      <ellipse cx="18" cy="60" rx="6" ry="4" fill={bodyColor} transform="rotate(-20, 18, 60)">
        {state === "active" && (
          <animateTransform attributeName="transform" type="rotate" values="-20 18 60;-30 18 60;-10 18 60;-20 18 60" dur="0.5s" repeatCount="indefinite" />
        )}
      </ellipse>
      <ellipse cx="82" cy="60" rx="6" ry="4" fill={bodyColor} transform="rotate(20, 82, 60)">
        {state === "active" && (
          <animateTransform attributeName="transform" type="rotate" values="20 82 60;30 82 60;10 82 60;20 82 60" dur="0.5s" repeatCount="indefinite" />
        )}
      </ellipse>
