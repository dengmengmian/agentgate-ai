...
export function SuperSoldierPet({ state }: Props) {
  ...
  const visorColor = state === "error" ? "#F85149" : state === "active" ? "#3FB950" : "#7C8CFF";
  ...
  return (
    <svg width="100" height="120" viewBox="0 0 100 120" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d="M28,50 Q18,70 22,100 Q30,105 38,98 L38,55Z" fill={capeColor}>
        {state === "active" && (
          <animate attributeName="d" values="M28,50 Q18,70 22,100 Q30,105 38,98 L38,55Z;M28,50 Q14,72 20,102 Q28,108 38,98 L38,55Z;M28,50 Q18,70 22,100 Q30,105 38,98 L38,55Z" dur="0.6s" repeatCount="indefinite" />
        )}
      </path>
      <path d="M62,55 L62,98 Q70,105 78,100 Q82,70 72,50Z" fill={capeColor}>
        {state === "active" && (
          <animate attributeName="d" values="M62,55 L62,98 Q70,105 78,100 Q82,70 72,50Z;M62,55 L62,98 Q72,108 80,102 Q86,72 72,50Z;M62,55 L62,98 Q70,105 78,100 Q82,70 72,50Z" dur="0.6s" repeatCount="indefinite" />
        )}
      </path>
      {/* Cape inner shadow */}
      <path d="M30,55 Q22,72 24,95 L34,92 L36,58Z" fill={capeDark} opacity="0.5" />
      <path d="M64,58 L66,92 L76,95 Q78,72 70,55Z" fill={capeDark} opacity="0.5" />

      {/* Helmet */}
      <path d="M30,18 Q30,4 50,4 Q70,4 70,18 L70,38 Q70,48 50,48 Q30,48 30,38Z" fill={armorColor} />
      {/* Helmet crest/ridge */}
      ...
      {/* Visor glow */}
      <rect x="36" y="24" width="28" height="10" rx="2" fill={visorColor} opacity="0.8">
        {state === "active" && (
      ...
      {/* Visor eyes slit */}
      {state === "sleep" ? (
        <line x1="38" y1="29" x2="62" y2="29" stroke="#111" strokeWidth="3" strokeLinecap="round" />
      ) : (
        <>
          <rect x="38" y="26" width="8" height="6" rx="1" fill="#111" opacity="0.6" />
          <rect x="54" y="26" width="8" height="6" rx="1" fill="#111" opacity="0.6" />
        </>
      )}
      ...
      {/* Chest plate lines */}
      <line x1="50" y1="48" x2="50" y2="80" stroke={armorDark} strokeWidth="1.5" />
      <path d="M35,56 L50,64 L65,56" stroke={armorDark} strokeWidth="1.5" fill="none" />

      {/* Chest emblem — star */}
      <polygon points="50,54 52,58 56,58 53,61 54,65 50,62 46,65 47,61 44,58 48,58" fill={visorColor}>
        {state === "active" && (
      ...
      {/* Shoulder pads */}
      <ellipse cx="26" cy="52" rx="10" ry="7" fill={armorColor} stroke={armorDark} strokeWidth="1" />
      <ellipse cx="74" cy="52" rx="10" ry="7" fill={armorColor} stroke={armorDark} strokeWidth="1" />
      {/* Shoulder spikes */}
      ...
      {state === "active" && (
        <>
          <circle cx="50" cy="60" r="20" fill="none" stroke={visorColor} strokeWidth="0.8" opacity="0.3">
      ...
      {state === "error" && (
        <>
          <path d="M42,58 L38,62 L41,64 L37,68" stroke="#F85149" strokeWidth="1" fill="none" strokeLinecap="round" />
          <path d="M60,56 L63,60 L59,63" stroke="#F85149" strokeWidth="1" fill="none" strokeLinecap="round" />
        </>
      )}
    </svg>
  );
