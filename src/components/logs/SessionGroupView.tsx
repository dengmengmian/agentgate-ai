          ...
          <thead>
            <tr className="border-b border-border text-text-muted">
              <th className="px-5 py-3 font-medium">
                {t("logs.session_col_session")}
              </th>
              <th className="px-5 py-3 font-medium">
                {t("logs.session_col_source")}
              </th>
              <th className="px-5 py-3 font-medium">
                {t("logs.session_col_last_seen")}
              </th>
              <th className="px-5 py-3 font-medium text-right">
                {t("logs.session_col_requests")}
              </th>
              <th className="px-5 py-3 font-medium text-right">
                {t("logs.session_col_in_out")}
              </th>
              <th className="px-5 py-3 font-medium text-right">
                ...
                <td className="px-5 py-2.5 font-mono text-text-primary">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setDeleteTarget(row.session_id);
                      }}
...
function SourceChip({ source }: { source: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded px-2 py-0.5 text-[10px] font-medium ${color}`}
    >
