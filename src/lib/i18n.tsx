...
const translations: Record<string, Record<Locale, string>> = {
  ...
  "providers.model_mapping_hint_v2": { en: "Only needed when the client model name differs from the upstream model. Native pass-through keeps the model name unchanged unless a mapping matches; protocol conversion uses mapping first, then falls back to the provider's default model for compatibility. For 1M Claude Code context, map the Claude model to a suffixed model such as mimo-v2.5-pro[1m] or deepseek-v4-pro[1m].", zh: "仅当客户端模型名和上游模型名不一致时需要。原生直通未命中映射时会保留模型名；协议转换会优先使用映射，未配置时用默认模型兜底以保持兼容。Claude Code 1M 上下文可把 Claude 模型映射到 mimo-v2.5-pro[1m] 或 deepseek-v4-pro[1m] 这类带后缀模型。" },
